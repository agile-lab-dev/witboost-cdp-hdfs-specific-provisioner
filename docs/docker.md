...
```bash
docker build -t cdp-private-hdfs-specific-provisioner .
```
...
```bash
docker run -d --name cdp-private-hdfs-sp-container -p 8888:8888 cdp-private-hdfs-specific-provisioner
```
...
-e OTEL_SERVICE_NAME=cdp-private-hdfs-specific-provisioner \
