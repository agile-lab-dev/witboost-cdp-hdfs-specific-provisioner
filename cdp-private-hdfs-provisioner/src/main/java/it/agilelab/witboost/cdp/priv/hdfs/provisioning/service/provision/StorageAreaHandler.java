...
public class StorageAreaHandler extends BaseHandler {
    ...
    private Either<FailedOperation, Void> deleteRangerEntities(String componentId) {
        var eitherPrefixes =
                buildPolicyPrefix(componentId)
                        .flatMap(
                                policyPrefix ->
                                        buildZoneName(componentId)
                                                .flatMap(
                                                        zoneName ->
                                                                buildUserRolePrefix(componentId)
                                                                        .map(
                                                                                userRolePrefix ->
                                                                                        new Tuple3<>(policyPrefix, zoneName, userRolePrefix))));
        ...
        var eitherPolicy =
                rangerService
                        .findPolicyByName(
                                rangerConfig.hdfsServiceName(), policyName(policyPrefix), Optional.of(zoneName))
                        .flatMap(
                                optP ->
                                        Option.ofOptional(optP).fold(() -> right(null), rangerService::deletePolicy));
        ...
        var eitherUserRole =
                rangerService
                        ...
                        .flatMap(
                                optR -> Option.ofOptional(optR).fold(() -> right(null), rangerService::deleteRole));
        if (eitherUserRole.isLeft()) return left(eitherUserRole.getLeft());

        return right(null);
    }

    private <T extends Specific> Either<FailedOperation, String> getPrefixPath(
            ProvisionRequest<T> provisionRequest) {
        String prefixPath;
    ...
    private <T extends Specific> Either<FailedOperation, Void> upsertRangerEntities(
        ...
        var eitherPrefixes =
                buildZoneName(provisionRequest.component().getId())
                        .flatMap(
                                zoneName ->
                                        buildOwnerRolePrefix(provisionRequest.component().getId())
                                                .flatMap(
                                                        ownerRolePrefix ->
                                                                buildUserRolePrefix(provisionRequest.component().getId())
                                                                        .flatMap(
                                                                                userRolePrefix ->
                                                                                        buildPolicyPrefix(provisionRequest.component().getId())
                                                                                                .flatMap(
                                                                                                        policyPrefix ->
                                                                                                                buildRangerPolicyFolderPath(
                                                                                                                                provisionRequest
                                                                                                                                        .component()
                                                                                                                                        .getId(),
                                                                                                                                prefixPath)
                        ...
                        .flatMap(
                                s ->
                                        Option.ofOptional(s)
                                                .fold(
                                                        () ->
                                                                rangerService.createSecurityZone(
                                                                        securityZone(
                                                                                zoneName,
                                                                                ...
                                                                                securityZoneFolderPath)),
                                                        sz ->
                                                                rangerService.updateSecurityZone(
                                                                        securityZone(
                                                                                sz,
                                                                                rangerConfig.hdfsServiceName(),
                                                                                securityZoneFolderPath))));
        ...
        var ownerRangerRoleRes =
                rangerService
                        .findRoleByName(ownerRoleName)
                        .flatMap(
                                r ->
                                        Option.ofOptional(r)
                                                .fold(
                                                        () ->
                                                                rangerService.createRole(
                                                                        rangerRole(ownerRoleName, ownerUsers, ownerGroups, deployUser)),
                                                        rr ->
                                                                rangerService.updateRole(rangerRole(rr, ownerUsers, ownerGroups))));
        if (ownerRangerRoleRes.isLeft()) return left(ownerRangerRoleRes.getLeft());

        String userRoleName = generateUserRoleName(userRolePrefix);
        var userRangerRoleRes =
                rangerService
                        .findRoleByName(userRoleName)
                        .flatMap(
                                r ->
                                                ...
                                                .fold(
                                                        ...
                                                        rr ->
                                                                rangerService.updateRole(
                                                                        rangerRole(
                                                                                rr, Collections.emptyList(), Collections.emptyList()))));
        if (userRangerRoleRes.isLeft()) return left(userRangerRoleRes.getLeft());

        var componentRangerPolicy =
                rangerService
                        .findPolicyByName(
                                rangerConfig.hdfsServiceName(), policyName(policyPrefix), Optional.of(zoneName))
                        .flatMap(
    ...
    private <T extends Specific>
            Either<FailedOperation, Tuple2<List<String>, List<String>>> mapOwners(
                    ProvisionRequest<T> provisionRequest) {
        var owners =
                Set.of(
        ...
        var problems =
                eitherPrincipals.values().stream()
    ...
    private Either<FailedOperation, String> buildSecurityZoneFolderPath(
    ...
    private Either<FailedOperation, String> buildRangerPolicyFolderPath(
            String componentId, String prefixPath) {
